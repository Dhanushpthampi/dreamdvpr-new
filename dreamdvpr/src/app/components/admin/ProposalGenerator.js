...
export default function ProposalGenerator({
    ...
    const handleSendToDrive = async () => {
        ...
        setSending(true);
        try {
            const fileName = `Proposal_${proposalData.client_name.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`;
            ...
            const data = await res.json();
            if (res.ok) {
                alert('Proposal sent to Google Drive successfully!');
            } else {
                alert(`Failed to send: ${data.error}`);
            }
        } catch (error) {
            console.error('Error sending to Drive:', error);
            alert('Failed to send proposal to Google Drive.');
        } finally {
            setSending(false);
        }
    };
